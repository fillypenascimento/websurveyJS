...
class ResultsController extends Controller
{
    ...
    public function createCsv(){
        ...
        $linhas = [[]];
        $linhas[0] = ["SQUARE ID", "SUBJECT ID", "QUESTION ID", "IS ATOM", "TIME", "CORRECT", "Experience", "Education Level", "Answered Everything", "REF", "EMPTY ANSWER", "ID IN SQUARE"];
        $counter = 1;
        foreach ($subjects as $subject) {
            foreach($subject->questions as $question){
                if($subject->square()){ // checking if user at least started to answer
                    ...
                    $linhas[$counter][8] = $subject->questions->count() == 12 ? "YES" : "NO";
                    $linhas[$counter][9] = $subject->ref;
                    $linhas[$counter][10] = $question->pivot->subject_answer == '' ? 'YES' : 'NO';
